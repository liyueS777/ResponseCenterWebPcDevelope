...
$(function () {
	...
	if (-1 == WebVideoCtrl.I_CheckPluginInstall()) {
                alert("您还未安装插件，请点击页面按钮下载，安装WebComponents.exe！");
               // downLoadPlugin();
		return;
	}
...
function clickLogin(cb) {
	var szIP = getQueryString('szIP'),
		szPort = getQueryString('szPort'),
		szUsername = getQueryString('szUsername'),
		szPassword = getQueryString('szPassword');
...
function downLoadPlugin() {
   window.location.href="../codebase.zip";
}
